 ...
 *	alloc_netdev_mq - allocate network device
 *	@sizeof_priv:	size of private data to allocate space for
 *	@buffer:	pointer to the memory buffer which has been allocated outside of alloc_netdev_mq(), buffer size = sizeof(net_device)+sizeof_priv+NETDEV_ALIGN_CONST
 *	@name:		device name format string
 *	@setup:		callback to initialize device
 *	@queue_count:	the number of subqueues to allocate
 *
 ...
 *	for each queue on the device at the end of the netdevice.
 */
struct net_device *alloc_netdev_mq_withbuf(int sizeof_priv, unsigned char *buffer, const char *name,
		void (*setup)(struct net_device *), unsigned int queue_count)
{
	struct netdev_queue *tx;
	struct net_device *dev;
...
EXPORT_SYMBOL(alloc_netdev_mq_withbuf);
#endif
/**
