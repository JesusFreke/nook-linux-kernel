...
extern struct platform_pcm_apm bravo_speaker_apm;
#endif
static int s3c24xx_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		bravo_speaker_apm.action = 1;
		schedule_work(&(bravo_speaker_apm.pm_work));
...
#ifdef CONFIG_MACH_BRAVO
extern struct platform_pcm_apm bravo_speaker_apm;
extern void bravo_speaker_pm(struct work_struct *w);
#endif	

static int __init s3c24xx_soc_platform_init(void)
{
	printk("s3c24xx_soc_platform_init\n");
#ifdef CONFIG_MACH_BRAVO
	memset(&bravo_speaker_apm, 0x00, sizeof(struct platform_pcm_apm));
	mutex_init(&(bravo_speaker_apm.ctrl_lock));
	INIT_WORK(&(bravo_speaker_apm.pm_work), bravo_speaker_pm);
#endif
