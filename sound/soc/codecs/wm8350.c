 ...
 * On Bravo, the digital and analogue supplies are the same, but
 * the headphone amplifier is separate.
 */
static const char *supply_names[] = {
	//"DCDC2"	// headphone amplifier
};
...
#ifdef CONFIG_MACH_BRAVO //Susan -- OUT2L/OUT2R are not connected in BRAVO, just return without PGA EVENT handling.
	if (w->connected == 0)
	{
		//printk(KERN_NOTICE "PGA_EVENT:w(%s) is not connected, return without handle PGA EVENTS\n", w->name);
		return 0;
	}
#endif
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (out) {
		...
		//Susan -- For Bravo, we need to setup volume configs in the actual registers (volume+VU bit), 
		// otherwise we don't have chance to config volume before stream start
...
static const struct snd_soc_dapm_widget wm8350_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA("Right Out1 PGA", WM8350_POWER_MGMT_3, 1, 0, NULL, 0),
	SND_SOC_DAPM_PGA("Left Out1 PGA", WM8350_POWER_MGMT_3, 0, 0, NULL, 0),
#else
...
#ifdef CONFIG_MACH_BRAVO
extern void bravo_speaker_check(void);
#endif
static int wm8350_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec = socdev->codec;
	//Note -- when system is on suspend path, No audio stream is playing back, so SYS_5V should have been turned off
...
#ifdef CONFIG_MACH_BRAVO
	//Note -- In case that Headphone is unplugged/plugged-in during system is in sleep mode
	bravo_speaker_check();
#endif
...
#ifdef CONFIG_MACH_BRAVO
	wm8350_clear_bits(wm8350, WM8350_POWER_MGMT_6, WM8350_DC2_ENA);
	printk(KERN_NOTICE "WM8350_PROBE:TURNOFF SYS_5V\n");
#endif
