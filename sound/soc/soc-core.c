...
#define soc_core_dev_dbg(dev, format, arg...)		\
...
static void soc_resume_deferred(struct work_struct *work)
{
	...
	soc_core_dev_dbg(socdev->dev, "starting resume work\n");
	...
	soc_core_dev_dbg(socdev->dev, "resume work completed\n");
...
static int soc_resume(struct platform_device *pdev)
{
	...
	soc_core_dev_dbg(socdev->dev, "scheduling resume work\n");
...
static void snd_soc_instantiate_card(struct snd_soc_card *card)
{
	...
	if (!found) {
		soc_core_dev_dbg(card->dev, "Platform %s not registered\n",
			card->platform->name);
		return;
	}
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (!found) {
			soc_core_dev_dbg(card->dev, "DAI %s not registered\n",
	...
	soc_core_dev_dbg(card->dev, "All components present, instantiating\n");
...
static int snd_soc_register_card(struct snd_soc_card *card)
{
	...
	soc_core_dev_dbg(card->dev, "Registered card '%s'\n", card->name);
...
static int snd_soc_unregister_card(struct snd_soc_card *card)
{
	...
	soc_core_dev_dbg(card->dev, "Unregistered card '%s'\n", card->name);
