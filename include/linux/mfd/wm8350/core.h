...
struct wm8350 {
	...
	/* Added by DavidB, this is the lcd backlight device */
	struct wm8350_lcd lcd;
	/*Added by Nas, this is for temperature monitor*/ 
	struct delayed_work monitor_work;
	struct delayed_work detect_work;
	/* Added by DavidB, this is the wake source */
	int wake_source;
        /* Added by GregC, for blink handling */
        struct delayed_work blink_work;
        int led_is_on;

    // Flag to track whether we're charging or not. The register on the PMIC 
    // is either unreliable or slow to update.
    int charging;

    // Added to skip interrupt processing while preparing to suspend.
    atomic_t suspend_on;
};
