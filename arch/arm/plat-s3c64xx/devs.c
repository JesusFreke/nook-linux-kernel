...
//Note: For Bravo, IRQ_EINT(10) is assigned to Headphone detection
...
//#define S3C64XX_SZ_USBHOST  0x00100000
...
/* Button interface */
static struct resource s3c_button_resource[] = {
	[0] = {
		.start = S3C_PA_KEYPAD,
		.end   = S3C_PA_KEYPAD+ S3C_SZ_KEYPAD - 1,
		.flags = IORESOURCE_MEM,
...
struct platform_device s3c_device_button = {
	.name		  = "s3c-button",
	.id		  = -1,
	.num_resources	  = ARRAY_SIZE(s3c_button_resource),
	.resource	  = s3c_button_resource,
};
EXPORT_SYMBOL(s3c_device_button);
...
struct platform_device s3c_eink_device = {
	.name		= "s1d13521fb",
	.id		= 0,
	.num_resources	= 0,
};
EXPORT_SYMBOL(s3c_eink_device);
...
static struct android_pmem_platform_data pmem_g3d_pdata = {
        .name = "pmem_g3d",
        .no_allocator = 0,
        .cached = 1,
};
...
static struct platform_device pmem_g3d_device = {
        .name = "android_pmem",
        .id = 7,
        .dev = { .platform_data = &pmem_g3d_pdata },
};
...
void __init s3c6410_add_mem_devices(struct s3c6410_pmem_setting *setting)
{
	...
	if (setting->pmem_g3d_size) {
                pmem_g3d_pdata.start = setting->pmem_g3d_start;
                pmem_g3d_pdata.size = setting->pmem_g3d_size;
                platform_device_register(&pmem_g3d_device);
        }
