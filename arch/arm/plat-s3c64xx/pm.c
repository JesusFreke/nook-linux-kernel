...
static void s3c_init_domain_power(void)
{
	...
	s3c_set_normal_cfg(S3C64XX_DOMAIN_P, S3C64XX_ACTIVE_MODE, S3C64XX_TVENC);
...
static int domain_off_check(unsigned int config)
{
...
{
	unsigned long tmp = __raw_readl(S3C_APLL_CON);
...
#endif	/* defined(CONFIG_S3C2410_PM_CHECK) && CONFIG_S3C2410_PM_CHECK_CHUNKSIZE != 0 */
...
/* s3c6410_pm_enter_mem
 *
 * central control for sleep/resume process
*/

static int s3c6410_pm_enter_mem(suspend_state_t state)
{
...
static int s3c6410_pm_enter(suspend_state_t state)
{
        switch (state) {
        case PM_SUSPEND_MEM:
                s3c6410_pm_enter_mem(state);
                break;
        }
        return 0;
}

static int s3c6410_pm_begin(suspend_state_t state) 
{
	int ret = 0;

	if (state == PM_SUSPEND_MEM) {
		/* Lock the voltage to max at this point */
		ret = lock_voltage(0);
	}
...
static int s3c6410_pm_end(suspend_state_t state) 
{
	/* Unlock the voltage to allow the governor to do it's thing */
	return unlock_voltage();
}

static struct platform_suspend_ops s3c6410_pm_ops = {
	.enter		= s3c6410_pm_enter,
	.valid		= suspend_valid_only_mem,
	.begin		= s3c6410_pm_begin,
	.end		= s3c6410_pm_end,
};
