...
static int __devinit sdhci_s3c_probe(struct platform_device *pdev)
{
    ...
    printk(KERN_NOTICE "sdhci_s3c_probe: return 0\n");
    return 0;
...
{
    struct s3c_sdhci_platdata *pdata = pdev->dev.platform_data;
    struct sdhci_host *s3c_host = pdata->host;
...
static int sdhci_s3c_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (pdev->id == 2) //SDIO controller has been powered off, relative SDIO func/card have been removed from SDIO stack
	{
		printk(KERN_NOTICE "Ignore SDIO controller suspend\n");
		return 0;
	}
#endif
...
#ifdef CONFIG_BRAVO_WIFI_ONOFF
	if (pdev->id == 2) //SDIO controller and relative SDIO func/card will be resumed and re-created by request from wpa supplicant
	{
		printk(KERN_NOTICE "Ignore SDIO controller suspend\n");
		return 0;
	}
...
#ifdef CONFIG_BRAVO_WIFI_ONOFF
int sdhci_s3c_suspend_sdio(struct platform_device *pdev, pm_message_t state)
{
    int i;
    struct s3c_sdhci_platdata *pdata = pdev->dev.platform_data;
    struct sdhci_host *s3c_host = pdata->host;
	...
	if ( (s3c_host) && (s3c_host->mmc) && (s3c_host->mmc->card) )
	{
		card = s3c_host->mmc->card;

		mmc_claim_host(s3c_host->mmc);

		for (i = 0; i < card->sdio_funcs; i++ )
		{
			if (card->sdio_func[i])
				sdio_func_set_removing(card->sdio_func[i]);
		}
		mmc_release_host(s3c_host->mmc);
	}

	if (pdev->id != 2) //SDIO controller has been powered off, relative SDIO func/card have been removed from SDIO stack
	{
		printk(KERN_NOTICE "Ignore SD/MMC controller suspend\n");
		return 0;
	}
...
int sdhci_s3c_resume_sdio(struct platform_device *pdev)
{
    struct s3c_sdhci_platdata *pdata = pdev->dev.platform_data;
    struct sdhci_host *s3c_host = pdata->host;
    int ret = 0;
    ...
    if (s3c_host) {
        ret = sdhci_resume_host(s3c_host);
	}
    return ret;
}

EXPORT_SYMBOL_GPL(sdhci_s3c_suspend_sdio);
EXPORT_SYMBOL_GPL(sdhci_s3c_resume_sdio);
#endif //CONFIG_BRAVO_WIFI_ONOFF
...
int sdhci_s3c_pdevid(struct mmc_host *host)
{
    //struct sdhci_host *p_sdhci_host = (struct sdhci_host *)(host->private);

    //if (p_sdhci_host)
    //{
    //    struct sdhci_s3c *p_sdhci_s3c = (struct sdhci_s3c *)(p_sdhci_host->private);

    //    if (p_sdhci_s3c)
    //        return p_sdhci_s3c->pdev->id;
    //}
