...
#if defined(CONFIG_MACH_CAPELA) || defined(CONFIG_MACH_BRAVO)
#include <linux/irq.h>
...
//#define DEBUG_S3C_KEVIN

#ifdef DEBUG_S3C_KEVIN
#define DEBUG_KEVIN(fmt,args...) printk(fmt, ##args)
#else
//evt-1 -- #define DEBUG_KEVIN(fmt,args...) do {} while(0)
#define DEBUG_KEVIN(fmt,args...) printk(fmt, ##args)
#endif
...
#if defined(CONFIG_MACH_CAPELA) || defined(CONFIG_MACH_BRAVO)
static int card_detect = 1;
static int movinand = 0;
#endif
...
static void sdhci_dumpregs(struct sdhci_host *host)
{
	...
	if (host->flags & SDHCI_USE_ADMA)
		printk(KERN_NOTICE DRIVER_NAME ": ADMA Err: 0x%08x | ADMA Ptr: 0x%08x\n",
		       readl(host->ioaddr + SDHCI_ADMA_ERROR),
		       readl(host->ioaddr + SDHCI_ADMA_ADDRESS));

	printk(KERN_NOTICE DRIVER_NAME ": ===========================================\n");
}
...
static void sdhci_init(struct sdhci_host *host)
{
	...
	intmask = SDHCI_INT_BUS_POWER | SDHCI_INT_DATA_END_BIT |
		...
		//SDHCI_INT_CARD_REMOVE | SDHCI_INT_CARD_INSERT | //For prototype|EVT1 board, there is not card detect pin for CH0/1/2
...
static void sdhci_tasklet_card(unsigned long param)
{
	...
	if (!card_detect) {
		...
		if (host->mrq) {
			...
			DEBUG_KEVIN("%s : host->mrq->cmd->error = -ENOMEDIUM;***\n",__FUNCTION__);
...
static void sdhci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	//[ICS]  HOST CONTROL REGISTER definition has been changed from S3C old verion (we are not sure which version) to S3C6410
	//So branch code like below
	#ifdef CONFIG_MACH_BRAVO  //BRAVO is based on S3C6410
	ctrl &= ~SDHCI_CTRL_OUTEDGEINV;
	#else
	...
	ctrl = readb(host->ioaddr + SDHCI_HOST_CONTROL);
	pr_debug("SDHCI_SET_IOS: SDHCI_HOST_CONTROL(0x%x)\n", ctrl);
...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	{
		pr_debug("sdhci_irq:(SDHCI_INT_CARD_INSERT | SDHCI_INT_CARD_REMOVE)\n");
		//mdelay(10);
		writel(intmask & (SDHCI_INT_CARD_INSERT | SDHCI_INT_CARD_REMOVE), host->ioaddr + SDHCI_INT_STATUS);
#ifdef CONFIG_MACH_BRAVO
		if (host->mmc->index == 1)  //On bravo board, only CH1 has card insert/removal tasklet
#endif
...
#ifdef CONFIG_MACH_BRAVO
	//if (cardint)
		//printk(KERN_NOTICE "SDHCI_INT_ENABLE(0x%x), SDHCI_SIGNAL_ENABLE(0x%x)\n", (readl(host->ioaddr + SDHCI_INT_ENABLE)), (readl(host->ioaddr + SDHCI_SIGNAL_ENABLE)));
	if ( (cardint) && ((readl(host->ioaddr + SDHCI_INT_ENABLE))&SDHCI_INT_CARD_INT) && ((readl(host->ioaddr + SDHCI_SIGNAL_ENABLE))&SDHCI_INT_CARD_INT) ) 
#else
	if (cardint)
#endif
...
int sdhci_resume_host(struct sdhci_host *host)
{
	...
	if (ret)
	{
		sdhci_reset(host, SDHCI_RESET_ALL);
		free_irq(host->irq, host);
		//S3C6410 sdhci_ops doesn't have (*enable_dma)() function pointer
...
int sdhci_add_host(struct sdhci_host *host)
{
	 ...
	 */
#ifdef CONFIG_MACH_BRAVO
	//slot 0 on CH0 is internal slot, don't need to card insert/removal tasklet
	//slot 2 are fixed to interface wifi chip, don't need card insert/removal tasklet
	if (mmc->index ==  1) 
...
#endif
	ret = request_irq(host->irq, sdhci_irq, IRQF_DISABLED, mmc_hostname(mmc), host);
