...
static int sdio_enable_wide(struct mmc_card *card)
{
	...
	if (ret)
		return ret;
	pr_debug("SDIO_CCCR_IF(0x%x)\n", ctrl);

#ifdef BRAVO_SDIO_1_BIT
	ctrl &= ~(3 << 0); //leave CCCR_BUS_INTERFACE register [bit1 bit0] as 0b00
#else
...
#ifdef BRAVO_SDIO_1_BIT
	pr_debug("SDIO_ENABLE_WIDE:MMC_SET_BUS_WIDTH\n");
...
int sdio_reset_comm(struct mmc_card *card)
{
	...
	return 0;
 err:
	pr_debug("%s: Error resetting SDIO communications (%d)\n",
	       mmc_hostname(host), err);
	mmc_release_host(host);
	return err;
}
