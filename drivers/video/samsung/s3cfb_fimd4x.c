...
s3cfb_fimd_info_t s3cfb_fimd = {
	.vidcon0 = S3C_VIDCON0_INTERLACE_F_PROGRESSIVE | S3C_VIDCON0_VIDOUT_RGB_IF | S3C_VIDCON0_L1_DATA16_SUB_16_MODE | \
			...
			S3C_VIDCON0_CLKVALUP_ALWAYS | S3C_VIDCON0_CLKDIR_DIVIDED | /*S3C_VIDCON0_CLKSEL_F_HCLK*/ S3C_VIDCON0_CLKSEL(0x3) | \
...
extern struct s3cfb_watchdog_control global_lcd_watchdog;
...
#endif // CONFIG_FB_S3C_DOUBLE_BUFFERING
...
irqreturn_t s3cfb_irq(int irqno, void *param)
{
	...
	// Reset LCD watchdog
	cancel_delayed_work(&global_lcd_watchdog.watchdog_control);
	schedule_delayed_work(&global_lcd_watchdog.watchdog_control, msecs_to_jiffies(500));
...
int s3cfb_init_registers(s3cfb_info_t *fbi)
{
	...
	/* Initialise LCD with values from here */
	local_irq_save(flags);

#if 0 //defined(CONFIG_MACH_BRAVO_EVT1)
	page_width = s3cfb_fimd.lcd_width * s3cfb_fimd.bytes_per_pixel;
	offset = (var->xres_virtual - s3cfb_fimd.lcd_width) * s3cfb_fimd.bytes_per_pixel;
	...
	printk("page_width: %ld offset: %ld\n", page_width, offset);
	...
	if (win_num == 0) {
		...
		lcd_clock = clk_get(NULL, "clk_27m");
		printk("lcd clock: 0x%lx pixel clock 0x%lx divider: 0x%x\n", clk_get_rate(lcd_clock), s3cfb_fimd.pixclock, S3C_VIDCON0_CLKVAL_F((int) ((clk_get_rate(lcd_clock) / s3cfb_fimd.pixclock) - 1)));
...
#if 0 // defined(CONFIG_MACH_BRAVO_EVT1)
    unsigned long xoffset, xwidth;
    int yoffset;

    /* Fixed offset is added to pan the start in the frame buffer */
    yoffset = fbi->fb.var.yoffset + s3cfb_fimd.fixed_yoffset;

    if (fbi->fb.var.yres_virtual - yoffset < s3cfb_fimd.lcd_height)
    {
        /* Don't allow moving beyond the end of the frame buffer*/
        yoffset = fbi->fb.var.yres_virtual - s3cfb_fimd.lcd_height;
    }

    start = fbi->fb.fix.line_length * yoffset;
#else  // CONFIG_MACH_BRAVO
...
#if 0 // defined(CONFIG_MACH_BRAVO_EVT1)
	xoffset = (fbi->fb.var.xres_virtual - s3cfb_fimd.lcd_width) * s3cfb_fimd.bytes_per_pixel;
	xwidth = s3cfb_fimd.lcd_width * s3cfb_fimd.bytes_per_pixel;
#endif // CONFIG_MACH_BRAVO
...
#if 0 // defined(CONFIG_MACH_BRAVO_EVT1)
        __raw_writel(S3C_VIDWxxADD2_OFFSIZE_F(xoffset) | (S3C_VIDWxxADD2_PAGEWIDTH_F(xwidth)), S3C_VIDW00ADD2);
#endif // CONFIG_MACH_BRAVO
        break;
...
int s3cfb_set_gpio(void)
{
	...
	printk("Setting up GPIO LCD interface\n");
	...
	/* backlight ON */
	//Question: On Bravo Prototype board, GPF15 is Not Connected at all.
	//Question: On Bravo EVT1 board, GPF15 is Not Connected at all.
	...
	//Susan -- on EVT1, LCD_RESET is GPN9
	/* module reset */
	if (gpio_is_valid(S3C64XX_GPN(9))) {
		err = gpio_request(S3C64XX_GPN(9), "GPN");

		if (err) {
			printk(KERN_ERR "failed to request GPN for EVT1 lcd reset control\n");
			return err;
		}
...
//	gpio_free(S3C64XX_GPN(9));
	...
	//Susan -- On Bravo Prototype board, LCD_DISP is GPP11
	/* module reset */
	if (gpio_is_valid(S3C64XX_GPP(11))) {
		err = gpio_request(S3C64XX_GPP(11), "GPP");

		if (err) {
			printk(KERN_ERR "failed to request GPP for "
				"lcd reset control\n");
			return err;
		}

		gpio_direction_output(S3C64XX_GPP(11), 1);
	}

	mdelay(100);

	gpio_set_value(S3C64XX_GPP(11), 0);
	mdelay(10);

	gpio_set_value(S3C64XX_GPP(11), 1);
	mdelay(10);

	gpio_free(S3C64XX_GPF(15));
	gpio_free(S3C64XX_GPP(11));
#endif
...
#ifndef CONFIG_HAS_WAKELOCK
#ifdef CONFIG_ANDROID_POWER
void s3cfb_early_suspend(android_early_suspend_t *h)
{
	s3cfb_info_t *info = container_of(h, s3cfb_info_t, early_suspend);
...
void s3cfb_late_resume(android_early_suspend_t *h)
{
	s3cfb_info_t *info = container_of(h, s3cfb_info_t, early_suspend);
...
#endif
#else	/* #ifndef CONFIG_HAS_WAKELOCK */
...
void s3cfb_early_suspend(struct early_suspend *h)
{
	...
	msleep(1);
	clk_disable(info->clk);
	atomic_set(&global_lcd_watchdog.active, 0); 
	cancel_delayed_work(&global_lcd_watchdog.watchdog_control);
}
...
#endif	/* end of #ifdef CONFIG_HAS_EARLYSUSPEND */
#endif	/* end of #ifndef CONFIG_HAS_WAKELOCK */
...
#endif	/* end of #ifndef CONFIG_HAS_EARLYSUSPEND */
#endif	/* end of #ifdef CONFIG_HAS_WAKELOCK */
