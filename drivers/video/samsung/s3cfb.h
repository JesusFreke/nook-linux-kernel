...
#define S3CFB_DEFAULT_BACKLIGHT_LEVEL	64
#else
...
#define S3C_FB_DEFAULT_BACKLIGHT_LEVEL 128 
/*
 *  macros
 */
...
#ifndef CONFIG_HAS_WAKELOCK
#ifdef CONFIG_ANDROID_POWER
	android_early_suspend_t	earlier_suspend;
...
typedef struct {
	...
	/* Fixed height offset value, used to pan lcd to the bottom */
	int fixed_yoffset;
	int lcd_height;
	int lcd_width;
	int lcd_xres;
	int lcd_yres;
	int visible_lcd_height;
...
struct s3cfb_watchdog_control
{
    s3cfb_info_t *s3cfb_info;
    atomic_t active;
    struct delayed_work watchdog_control;
};
...
#ifdef CONFIG_FB_S3C_PICTURETUBES
extern void s3cfb_init_hw(struct device *dev);
#else
...
#ifndef CONFIG_HAS_WAKELOCK
#ifdef CONFIG_ANDROID_POWER
extern void s3cfb_earlier_suspend(android_early_suspend_t *h);
extern void s3cfb_early_suspend(android_early_suspend_t *h);
extern void s3cfb_late_resume(android_early_suspend_t *h);
#endif
#else
#ifdef CONFIG_HAS_EARLYSUSPEND
extern void s3cfb_earlier_suspend(struct early_suspend *h);
