...
int lbs_scan_networks(struct lbs_private *priv, int full_scan)
{
    ...
    // Don't signal carrier_off here. This zeroes the IFF_RUNNING
    // flag on the interface and the clients assume the carrier 
    // was lost. 
	if (priv->mesh_dev) {
		 ...
		 union iwreq_data *wrqu, char *extra)
{
	...
	/* allow scanning when net interface is down
 ...
 *  @return        0 or -1
 */
int lbs_ret_80211_scan(struct lbs_private *priv, unsigned long dummy,
...
int BgScanQueryCb(struct lbs_private* priv, unsigned long dummy,
    struct cmd_header* resp)
{
    rsp_ds_802_11_bg_scan_query* scanResp = (rsp_ds_802_11_bg_scan_query*)resp;
	struct bss_descriptor *iter_bss;
	struct bss_descriptor *safe;
	uint8_t *bssinfo;
	uint16_t scanrespsize;
    ...
    //DumpData(scanResp, scanResp->hdr.size);
    lbs_deb_scan( "Number of BG scan results = %d\n", scanResp->numOfSet);
	...
	if (scanResp->numOfSet > MAX_NETWORK_COUNT) {
		lbs_deb_scan("SCAN_RESP: too many scan results(%d, max %d)\n",
			     scanResp->numOfSet, MAX_NETWORK_COUNT);
		ret = -1;
		goto done;
	}

	bytesleft = le16_to_cpu(scanResp->bufSize);
	lbs_deb_scan("SCAN_RESP: bssdescriptsize %d\n", bytesleft);

	scanrespsize = le16_to_cpu(resp->size);
	lbs_deb_scan("SCAN_RESP: scan results %d\n", scanResp->numOfSet);

	bssinfo = scanResp->bssDescSet;

	/*
	 *  Process each scan response returned (scanResp->numOfSet). Save
	 *    the information in the newbssentry and then insert into the
	 *    driver scan table either as an update to an existing entry
	 *    or as an addition at the end of the table
	 */
	for (idx = 0; idx < scanResp->numOfSet && bytesleft; idx++) {
		struct bss_descriptor new;
		struct bss_descriptor *found = NULL;
		struct bss_descriptor *oldest = NULL;
		DECLARE_MAC_BUF(mac);
    ...
    // Notify supplicant that the scan results are ready. The supplicant should
    // connect to an SSID. If for some reason it doesn't, then it can issue a
    // request to enter Deep Sleep with Background scan again.
	memset(&wrqu, 0, sizeof(union iwreq_data));
	wireless_send_event(priv->dev, SIOCGIWSCAN, &wrqu, NULL);
...
void DumpData(u8* resp, int length)
{
    int i;
