...
{
    // Don't do IRQ processing if preparing to suspend. This can't be done 
    // until after the status registers have been read to clear the interrupt 
    // in wm8350_irq_worker().
    if (atomic_read(&wm8350->suspend_on)) {
        return;
    }

    mutex_lock(&wm8350->irq_mutex);

	if (!wm8350->wake_source) {
		wm8350->wake_source = irq;
	}
...
#ifdef CONFIG_MACH_BRAVO
static uint32_t g_wm8350_reg;

static ssize_t wm8350_r_val(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "0x%x\n",  wm8350_reg_read(dev->driver_data, g_wm8350_reg));
}

static ssize_t wm8350_w_val(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    wm8350_reg_write(dev->driver_data, g_wm8350_reg, val);

    return count;
}

static DEVICE_ATTR(val, S_IWUSR | S_IRUGO, wm8350_r_val, wm8350_w_val);

static ssize_t wm8350_r_reg(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "0x%.2x\n", g_wm8350_reg);
}

static ssize_t wm8350_w_reg(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned long val = simple_strtoul(buf, NULL, 16);
...
static DEVICE_ATTR(reg, S_IWUSR | S_IRUGO, wm8350_r_reg, wm8350_w_reg);

static ssize_t wm8350_show_wake_source(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct wm8350 *wm8350 = dev_get_drvdata(dev);

	switch(wm8350->wake_source) {
	case WM8350_IRQ_EXT_USB_FB:
		ret = sprintf(buf, "usb\n");
		break;
	...
	case WM8350_IRQ_RTC_ALM:
		ret = sprintf(buf, "rtc_alarm\n");
		break;

	default:
		ret = sprintf(buf, "unknown: %d\n", wm8350->wake_source);
		break;
	}
...
static DEVICE_ATTR(wake_source, S_IRUGO, wm8350_show_wake_source, NULL);

void wm8350_save_wake_source(struct wm8350 *wm8350) 
{
	wm8350->wake_source = 0;
}

static struct device_attribute* const wm8350_attr[] = {
    &dev_attr_reg,
    &dev_attr_val,
	&dev_attr_wake_source,
};

#endif

/**
    This should be in the platform device list but
    there isn't really any good place to put it.
*/

static struct wm8350_led_platform_data isink_a_data = {
    .name = "lcd-backlight",
    .default_trigger = "active",
    .max_uA = 40000,
};
               ...
               struct wm8350_platform_data *pdata)
{
    ...
    dev_info(wm8350->dev, "WM8350 configured IRQ(%d)\n", irq);
    wm8350->chip_irq = irq;
    ...
    // Added by DavidB, this will register a lcd led backlight.
    // LCD Ramp on/off: 0.5s
    ret = wm8350_isink_set_flash(wm8350, WM8350_ISINK_A, 0, 0, 0, 0x2, 0x20, WM8350_CS1_DRIVE);

    if (ret < 0) {
        dev_err(wm8350->dev, "Failed to set flash mode: %d\n", ret);
    }

    ret = wm8350_dcdc25_set_mode(wm8350, WM8350_DCDC_5, WM8350_DC5_MODE_BOOST,
                WM8350_DC5_ILIM_LOW, WM8350_DC5_RMP_10V, WM8350_DC5_FBSRC_ISINKA);

    if (ret < 0) {
        dev_err(wm8350->dev, "Failed to set dcdc mode: %d\n", ret);
    }

    ret = wm8350_dcdc_set_slot(wm8350, WM8350_DCDC_5, 0xf, 0xf,  WM8350_DC5_ERRACT_NONE);

    if (ret < 0) {
        dev_err(wm8350->dev, "Failed to set dcdc slot: %d\n", ret);
    }

    ret = wm8350_register_led(wm8350, 0, WM8350_DCDC_5, WM8350_ISINK_A, &isink_a_data);

    if (ret < 0) {
        dev_err(wm8350->dev, "Failed to register led: %d\n", ret);
    }

    //Susan -- the last step, as device_attr for wm8350 testing
#ifdef CONFIG_MACH_BRAVO
    for (i = 0; i < ARRAY_SIZE(wm8350_attr); i++)
    {
        if ( (ret = device_create_file(wm8350->dev, wm8350_attr[i])) < 0)
        {
            while (i--)
            {
                 device_remove_file(wm8350->dev, wm8350_attr[i]);
            }
            goto err;
        }
    }
