...
{
	int			status = 0;
	printk("ohci_rh_suspend START\n");
	
	ohci->hc_control = ohci_readl (ohci, &ohci->regs->control);
	switch (ohci->hc_control & OHCI_CTRL_HCFS) {
	case OHCI_USB_RESUME:
		printk("resume/suspend?\n");
...
__acquires(ohci->lock)
{
	...
	if (ohci->hc_control & (OHCI_CTRL_IR | OHCI_SCHED_ENABLES)) {
		...
		if (hcd->state == HC_STATE_RESUMING) {
			printk("BIOS/SMM active, control %03x\n",
					ohci->hc_control);
			status = -EBUSY;
		/* this happens when pmcore resumes HC then root */
		} else {
			printk("duplicate resume\n");
			status = 0;
		}
	...
	case OHCI_USB_RESUME:
		...
		printk("%swakeup root hub\n",
				autostopped ? "auto-" : "");
		break;
	case OHCI_USB_OPER:
		/* this can happen after resuming a swsusp snapshot */
		printk("snapshot resume? reinit\n");
		status = -EBUSY;
		break;
	...
	if (temp != OHCI_USB_RESUME) {
		printk( "controller won't resume\n");
		spin_lock_irq(&ohci->lock);
		return -EBUSY;
	}
...
skip_resume:
	...
	if (enables) {
		printk("restarting schedules ... %08x\n", enables);
...
static void ohci_finish_controller_resume(struct usb_hcd *hcd)
{
	...
	printk("ohci_finish_controller_resume START\n");
	...
	printk("ohci_finish_controller_resume END\n");
}
