...
//#define DEBUG
#define POWER_LEVEL_AUTO  1
#define DRIVER_VERSION "1.7.17b"
...
/* copied from 2.6.28 */
static struct urb *usb_get_from_anchor(struct usb_anchor *anchor)
{
	struct urb *victim;
	unsigned long flags;
...
/* copied from 2.6.28 */
static void usb_scuttle_anchored_urbs(struct usb_anchor *anchor)
{
	struct urb *victim;
	unsigned long flags;
			...
			const struct usb_device_id *id)
{
	...
	if (is_blacklisted(ifnum, (struct sierra_iface_info *)id->driver_info)) {
...
int usb_autopm_get_interface_async(struct sierra_intf_private *intfdata)
{
	atomic_inc(&intfdata->stats.async_gets);
	atomic_inc(&intfdata->gets);
	return schedule_work(&intfdata->autopm_kevent);
}
int usb_autopm_put_interface_async(struct sierra_intf_private *intfdata)
{
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 4096, "%i %i %i\n",
			flag_suspended,
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 4096,
		...
		atomic_read(&intfdata->stats.write_cnt), atomic_read(&intfdata->stats.delayed_writes),
...
static int sierra_create_sysfs_attrs(struct usb_serial_port *port)
{
	device_create_file(&port->dev, &dev_attr_stats);
...
static void sierra_outdat_callback(struct urb *urb)
{
	...
	usb_autopm_put_interface_async(intfdata);
					...
					const unsigned char *buf, int count)
{
	...
	retval = usb_autopm_get_interface_async(intfdata);
...
static void sierra_instat_callback(struct urb *urb)
{
	...
	if (status == 0) {
		...
		const u8 * sigp = (u8 *)(req_pkt + 1);
		/* usb_ctrlrequest is followed by at least one byte of data 
		 * make sure we received that many bytes
		 */
		if (urb->actual_length >= sizeof(*req_pkt) + sizeof(*sigp) &&
		   req_pkt->bRequestType == 0xA1 && req_pkt->bRequest == 0x20) {
			int old_dcd_state;
			const u8 signals = *sigp;
...
static int sierra_startup(struct usb_serial *serial)
{
	...
	if (serial->num_ports) {
		portdata = kzalloc(sizeof(*portdata) * serial->num_ports,
					GFP_KERNEL);
		if (!portdata) {
	...
	if (result == sizeof(fw_attr) && (fw_attr & 0x01) ) {
