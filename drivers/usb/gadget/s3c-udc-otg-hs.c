...
int s3c_udc_soft_isconnected(void);
...
static struct s3c_udc	*the_controller = NULL;
...
#ifdef CONFIG_USBHOST_CLOCK_EPLL
static int udc_enable(struct s3c_udc *dev, int no_analog);
#else
...
struct usb_ctrlrequest __attribute__((__aligned__(4))) ctrl;
//---------------------------------------------------------------------------------------
...
static int udc_enable(struct s3c_udc *dev, int no_analog)
{
	...
	/* Power On OTG Block and Analog Power Block */
	if (no_analog)
		writel((1<<3), S3C_USBOTG_PHYPWR);  //Power on OTG block, force_suspend disable
	else
		writel(0x0, S3C_USBOTG_PHYPWR); //Power on OTG block, Analog block, force_suspend disabled

    /* Configure OTG PHY Clocks:
     *   - D+ and D- data Tx and Rx through UTMI
     *   - Reference Clock = external crystal
     *   - 48 MHz Clock available in Suspend
     *   - Analog ID Sample disabled
     *   - PLL Clock Frequency: 12 MHz
     */
	writel(0x2, S3C_USBOTG_PHYCLK);  //OPHYCLK[xo_ext_clk_enb(5)] --0(external crystal), OPHYCLK[common_on_n(4)] -- 0, OPHYCLK[clk_sel(1:0)] -- 2 (external 12MHz)
	...
	printk(KERN_NOTICE "UDC_ENABLE->reconfig_usbd()\n");
	reconfig_usbd(dev);
...
void otg_dump_regs(void)
{
	printk(KERN_NOTICE "Current Time(%d)\n", jiffies);
	printk(KERN_NOTICE "S3C_OTHERS(0x%x), OPHYPWR(0x%x), OPHYCLK(0x%x), RSTCON(0x%x)\n", (readl(S3C_OTHERS)), (readl(S3C_USBOTG_PHYPWR)), (readl(S3C_USBOTG_PHYCLK)), (readl(S3C_USBOTG_RSTCON)));
}
EXPORT_SYMBOL_GPL(otg_dump_regs);
#else
static int udc_enable(struct s3c_udc *dev)
{
	u32 uTemp;
	...
	/* Power On OTG Block and Analog Power Block */
	writel(0x0, S3C_USBOTG_PHYPWR);  //Power on OTG block, power on analog block, force_suspend disable
...
static void udc_disable(struct s3c_udc *dev)
{
	...
	dev->gadget.speed = USB_SPEED_UNKNOWN;
	dev->usb_address = 0;
	//Susan -- below operation doesn't match with 6410 DATASHEET, comment it out
	//writel(readl(S3C_USBOTG_PHYPWR)|(0x7<<1), S3C_USBOTG_PHYPWR);
#ifdef CONFIG_USBHOST_CLOCK_EPLL
	local_irq_save(flags);
	writel(readl(S3C_USBOTG_PHYPWR)|(0x1b), S3C_USBOTG_PHYPWR);
	writel(readl(S3C_HCLK_GATE)&~(S3C_CLKCON_HCLK_USB), S3C_HCLK_GATE);
	writel(readl(S3C_OTHERS) & ~S3C_OTHERS_USB_SIG_MASK, S3C_OTHERS);
	local_irq_restore(flags);
#else
	writel(readl(S3C_USBOTG_PHYPWR)|(1<<4), S3C_USBOTG_PHYPWR);
#endif
...
#ifdef CONFIG_USBHOST_CLOCK_EPLL
extern int last_detected_supply;
int s3c_udc_suspend(struct platform_device *pdev, pm_message_t state);
...
#ifdef CONFIG_USBHOST_CLOCK_EPLL
	if (last_detected_supply != WM8350_USB_SUPPLY)  // 2 -- WM8350_USB_SUPPLY
	{
...
#ifdef CONFIG_USBHOST_CLOCK_EPLL
	udc_enable(dev, 0);  //don't power up ANALOG block
#else
	udc_enable(dev);  //don't power up ANALOG block
#endif
}
EXPORT_SYMBOL_GPL(s3c_udc_power_up);
//---------------------------------------------------------------------------------------
...
int s3c_udc_soft_isconnected(void)
{
	...
	return !(uTemp & SOFT_DISCONNECT);
}
EXPORT_SYMBOL(s3c_udc_soft_isconnected);
...
//---------------------------------------------------------------------------------------
struct s3c_udc* s3c_get_addr()
{
	return &memory; 
}
EXPORT_SYMBOL(s3c_get_addr);
/*
...
#ifdef CONFIG_USBHOST_CLOCK_EPLL
EXPORT_SYMBOL_GPL(s3c_udc_suspend);
#endif
//---------------------------------------------------------------------------------------
#ifdef CONFIG_USBHOST_CLOCK_EPLL
int s3c_udc_resume(struct platform_device *pdev)
#else
...
#ifdef CONFIG_USBHOST_CLOCK_EPLL
EXPORT_SYMBOL_GPL(s3c_udc_resume);
#endif
//---------------------------------------------------------------------------------------
#endif /* CONFIG_PM */
