...
static const struct rtc_class_ops wm8350_rtc_ops = {
    ...
    .irq_set_state = wm8350_rtc_alarm_irq_enable,
    .irq_set_state = wm8350_rtc_update_irq_enable,
};
...
static void wm8350_rtc_shutdown(struct platform_device *pdev)
{
	struct wm8350 *wm8350 = platform_get_drvdata(pdev);
	wm8350_rtc_stop_alarm(wm8350);
}
#endif
...
static int __devexit wm8350_rtc_remove(struct platform_device *pdev)
{
	...
	/* For the bravo device we want the alarms to be off
		when we turn off */
#ifdef CONFIG_MACH_BRAVO
	wm8350_rtc_stop_alarm(wm8350);
#endif
