		...
		int cmd, unsigned long arg)
{
	...
	static s3c_jpeg_t			*s3c_jpeg_buf;
	...
	switch (cmd) 
	{
		case IOCTL_JPG_DECODE:
			...
			JPEG_Copy_From_User(&DecReturn, (JPG_DEC_PROC_PARAM *)arg, sizeof(JPG_DEC_PROC_PARAM));
			...
			if(!JPGMem.v_pJPGData_Buff) {
				printk(KERN_INFO "No allocate physical mem\n");
				return -1;
			}
			...
			JPEG_Copy_To_User((void *)arg, (void *)&DecReturn, sizeof(JPG_DEC_PROC_PARAM));
			break;
...
/*		case IOCTL_JPG_GET_STRBUF:
		...
		case IOCTL_JPG_SET_STRBUF:
			s3c_jpeg_buf = (s3c_jpeg_t *)kmalloc(sizeof(s3c_jpeg_t), GFP_KERNEL);
			
			if(!s3c_jpeg_buf) {
				JPEG_LOG_MSG(LOG_ERROR, "s3c_jpeg_ioctl", "DD::fail memory allocate\n");
				return FALSE;
			}
...
/*		case IOCTL_JPG_GET_THUMB_STRBUF:
			JPEG_LOG_MSG(LOG_TRACE, "s3c_jpeg_ioctl", "IOCTL_JPG_GET_THUMB_STRBUF\n");
			UnlockJPGMutex();
...
int s3c_jpeg_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if( remap_pfn_range(vma, vma->vm_start, pageFrameNo, size,	\
				vma->vm_page_prot) ) {
		JPEG_LOG_MSG(LOG_ERROR, "s3c_jpeg_mmap", "jpeg remap error");
		return -EAGAIN;
	}
