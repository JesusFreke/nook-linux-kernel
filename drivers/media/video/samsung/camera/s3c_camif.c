...
int s3c_camif_get_frame_num(camif_cfg_t *cfg)
{
	...
	//printk("###### kcoolsw : s3c_camif_get_frame_num : %d\n", cfg->cur_frame_num);
...
static int s3c_camif_output_pp_codec_rgb(camif_cfg_t *cfg)
{
	...
	else
	{
		...
		case 4:
				...
				{
					each_area = area * i;
					cfg->img_buf[i].virt_rgb = cfg->pp_virt_buf + each_area;
					cfg->img_buf[i].phys_rgb = cfg->pp_phys_buf + each_area;
				writel(cfg->img_buf[i].phys_rgb, cfg->regs + S3C_CICOYSA(i));
			}
...
static int s3c_camif_output_pp_codec(camif_cfg_t *cfg)
{
	...
	for (i = 0; i < MAX_PPNUM; i++)
	{
		...
		{
			//printk("######### kcoolsw 2: cfg->img_buf[%d].phys_y : %d \n", i, cfg->img_buf[i].phys_y);
		...
		{
			//printk("######### kcoolsw 3: cfg->img_buf[%d].phys_y : %d \n", i, cfg->img_buf[i].phys_y);
...
#else
	writel((readl(cfg->regs + S3C_CICOCTRL) & ~(0x3 << 0)), cfg->regs + S3C_CICOCTRL);
#endif
#endif
