...
static void s3c_tvscaler_set_clk_src(scaler_clk_src_t clk_src)
{
    ...
    //printk("  [%s] S3C_MODE[22:21] = %d\n", __FUNCTION__, (tmp >> 21) & 0x3);
 //   if ((tmp >> 23) & 0x1)
 //       printk("  [%s] S3C_MODE[29:24] = %d\n", __FUNCTION__, ((tmp >> 24) & 0x1f) + 1);
...
{
    int i;
    u32 tmp = __raw_readl(base + S3C_MODE);

    //printk("  [%s] MODE = 0x%08x\n", __FUNCTION__, tmp);

    __raw_writel(S3C_POSTENVID_DISABLE, base + S3C_POSTENVID);

    tmp = __raw_readl(base + S3C_MODE);

    tmp &= ~(0x1<<31);     // must be 0
    tmp &= ~(0x1<<13);     // dma
    tmp &= ~(0x1<<14);     // per frame mode
    ...
    if (interruptible_sleep_on_timeout(&waitq, 100) == 0)
    {
        printk(KERN_ERR "\n%s: Waiting for interrupt timedout\n", __FUNCTION__);
    }
#if 0
    //printk("  [%s] POSTENVID start = 0x%08x\n", __FUNCTION__, tmp);

    while (1)
    //for (i = 0; i < 100; i++)
    {
        tmp = __raw_readl(base + S3C_POSTENVID);
        if ((tmp & S3C_POSTENVID_ENABLE) == 0)
            break;
        //printk("  [%s] POSTENVID = 0x%08x\n", __FUNCTION__, tmp);
        msleep(10);
    }
    //printk("  [%s] POSTENVID done = 0x%08x\n", __FUNCTION__, tmp);
#endif
}
...
// peter mod. end
    ...
    //s3c_tvscaler_set_clk_src(PLL_EXT);
