...
int MFCInst_Encode(MFCInstCtx *ctx, int *enc_data_size, int *header_size, int *sps, int *pps)
{
	volatile S3C6400_MFC_PARAM_REG_ENC_PIC_RUN	*pPARAM_ENC_PIC_RUN;
	S3C6400_MFC_SFR						*mfc_sfr;

	int                                hdr_size;
	int                                hdr_size1 = 0, hdr_size2 = 0;	
	...
	if (MFCINST_STATE_CHECK(ctx, MFCINST_STATE_ENC_INITIALIZED)) {
		...
		if (ctx->codec_mode == MP4_ENC) {
			...
			//  ENC_HEADER command  //
			MFCInst_EncHeader(ctx, 0, 0, ctx->phyadrStrmBuf, ctx->nStrmBufSize, &hdr_size1);	// SPS
			MFCInst_EncHeader(ctx, 1, 0, ctx->phyadrStrmBuf + (hdr_size1 + 3), ctx->nStrmBufSize-(hdr_size1+3), &hdr_size2);	// PPS

			LOG_MSG(LOG_TRACE, "MFCInst_Encode", "hdr_size1=%d\r\n", hdr_size1);
			LOG_MSG(LOG_TRACE, "MFCInst_Encode", "hdr_size2=%d\r\n", hdr_size2);
			
			// Backup the stream header in the temporary header buffer.
			hdr_buf_tmp = (unsigned char *) Mem_Alloc(hdr_size1 + 3 + hdr_size2);
			Mem_Cpy(hdr_buf_tmp, ctx->pStrmBuf, hdr_size1);

                         // from 2.8.5
			//dmac_clean_range(ctx->pStrmBuf, (ctx->pStrmBuf + hdr_size1));
			//dmac_flush_range(ctx->pStrmBuf, (ctx->pStrmBuf + hdr_size1));
			Mem_Cpy(hdr_buf_tmp + hdr_size1, (unsigned char *)((unsigned int)(ctx->pStrmBuf + (hdr_size1 + 3)) & 0xFFFFFFFC), hdr_size2);

			hdr_size = hdr_size1 + hdr_size2;
