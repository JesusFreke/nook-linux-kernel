...
static void s3c_camif_change_mode(camif_cfg_t *cfg, int mode)
{
	...
	switch (res) {
	case SENSOR_QSXGA:
		printk(KERN_INFO "Resolution changed into QSXGA (2592x1944) mode\n");
	...
	case SENSOR_QXGA:
		printk(KERN_INFO "Resolution changed into QXGA (2048x1536) mode\n");
	...
	case SENSOR_UXGA:
		printk(KERN_INFO "Resolution changed into UXGA (1600x1200) mode\n");
	...
	case SENSOR_SXGA:
		printk(KERN_INFO "Resolution changed into SXGA (1280x1024) mode\n");
	...
	case SENSOR_XGA:
		printk(KERN_INFO "Resolution changed into XGA (1024x768) mode\n");
	...
	case SENSOR_SVGA:
		printk(KERN_INFO "Resolution changed into SVGA (800x600) mode\n");
	...
	case SENSOR_VGA:
		printk(KERN_INFO "Resolution changed into VGA (640x480) mode\n");
...
int s3c_camif_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VIDIOC_S_DIG_ZOOM:
		...
		ret = s3c_camif_restart_preview(cfg);
		break;
...
#ifdef CONFIG_VIDEO_SAMSUNG_S5KCA
