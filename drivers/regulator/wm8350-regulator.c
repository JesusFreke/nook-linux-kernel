...
static int wm8350_regulator_probe(struct platform_device *pdev)
{
	...
	init_data = (struct regulator_init_data *) pdev->dev.platform_data;
	ret = regulator_set_machine_constraints(wm8350_reg[pdev->id].name, &init_data->constraints);
	
	if (ret < 0) {
		dev_err(&pdev->dev, "failed to set machine constraints: %d\n", ret);
